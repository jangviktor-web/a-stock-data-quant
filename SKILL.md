...
> 2026-08-26 用真实 Key 跑 12 维 live test：**全部 HTTP 200 + `code==0`，返回真实当前数据**（交易日 2026-08-25，茅台 ¥1304 / 沪深300 4552.03，龙虎榜 `trade_date=2026-08-25`）。覆盖与结果：
> | 维度 | 结果 |
> | --- | --- |
> | 元信息检索 / 行情快照 / 历史日K(242 根) / 复权因子(30 事件) / 估值快照 / 利润表(4 期) | PASS |
> | 指数目录(390) / 指数快照 / 基金资料(沪深300A·华夏) | PASS |
> | 涨停池(今日) / 龙虎榜 / 连板梯队(30 日) | PASS |
> **12/12 PASS**。已知小瑕疵（上游数据，非契约违约）：指数快照 `ticker` 对沪深300返回 `1B0300`（应为 `000300`）——取数时一律以 `thscode` 为唯一键，不依赖 `ticker`。Key 仅运行时内存传入（env），未落盘 / 未进 Git / 未进日志。
