...
def _dc_zt_pool(date='', limit=30):
    """东财 push2 涨停池备用源"""
    import requests as _req
    date = _recent_trade_date(date)
    url = 'https://push2ex.eastmoney.com/getTopicZTPool'
    params = {'ut': '7eea3edcaed734bea9cbfc24409ed989', 'dpt': 'wz.ztzt', 'date': date}
    s = _req.Session()
    s.trust_env = False
    r = s.get(url, params=params, timeout=10)
    d = r.json()
    pool = (d.get('data') or {}).get('pool', [])
    rows = []
    for item in pool[:limit]:
        rows.append({
            ...
            'price': item.get('p', 0) / 1000 if item.get('p') else 0,
            'amount': item.get('amount', 0),
            'turnover': item.get('hs', 0),
            'seal_amount': item.get('fund', 0),
            'first_seal': str(item.get('fbt', '')),
            'last_seal': str(item.get('lbt', '')),
            'break_count': item.get('zbc', 0),
            'zt_stat': '',
            'streak': item.get('lbc', 0),
            'industry': str(item.get('hybk', '')),
        })
    return rows


@_with_fallback(_ak_zt_pool, ('东财push2', _dc_zt_pool))
...
def _dc_dt_pool(date='', limit=30):
    ...
    for item in pool[:limit]:
        rows.append({
            ...
            'turnover': item.get('hs', 0),
            'seal_amount': item.get('fund', 0),
            'consecutive': item.get('lbc', 0),
            'industry': str(item.get('hybk', '')),
        })
    return rows


@_with_fallback(_ak_dt_pool, ('东财push2', _dc_dt_pool))
...
def _ak_sector_fund_rank(days='今日', category='行业资金流', limit=20):
    """akshare 板块资金流排名"""
    _require_akshare()
...
def _dc_sector_fund_rank(days='今日', category='行业资金流', limit=20):
    """东财 push2 板块资金流备用源"""
    import requests as _req
    # 板块类型映射
    type_map = {'行业资金流': '2', '概念资金流': '3', '地域资金流': '1'}
    # 时间映射
    time_map = {'今日': '1', '5日': '5', '10日': '10'}
    bt = type_map.get(category, '2')
    td = time_map.get(days, '1')
    ...
    params = {
        ...
        'fltt': 2, 'invt': 2,
        'fid0': 'f4001', 'fs': f'm:90+t:{bt}',
        'fid': 'f62', 'fields': 'f12,f14,f3,f62,f184,f66,f69,f72,f75',
    }
    s = _req.Session()
    s.trust_env = False
    r = s.get(url, params=params, timeout=10)
    ...
    for item in items[:limit]:
        rows.append({
            ...
            'large_net': item.get('f72', 0),
        })
    return rows


@_with_fallback(_ak_sector_fund_rank, ('东财push2', _dc_sector_fund_rank))
