...
def get_price(code, end_date='',count=10, frequency='1d', fields=[]):        #对外暴露只有唯一函数，这样对用户才是最友好的
    ...
    if  frequency in ['1d','1w','1M']:   #1d日线  1w周线  1M月线
         try:    return get_price_sina( xcode, end_date=end_date,count=count,frequency=frequency)   #主力
             ...
             try: return get_price_day_tx(xcode,end_date=end_date,count=count,frequency=frequency)   #备用
             except:
                 try:                                                           #mootdx备用
                     from lib.sources_mootdx import get_kline
                     freq_map = {'1d': 9, '1w': 10, '1M': 11}
                     return get_kline(xcode, frequency=freq_map.get(frequency, 9), offset=count)
                 except:
                     try:                                                       #百度备用
                         from lib.sources_baidu import get_kline
                         return get_kline(xcode, count=count)
                     except:
                         raise RuntimeError(f"所有数据源均失败: {code}")
    ...
    if  frequency in ['1m','5m','15m','30m','60m']:  #分钟线 ,1m只有腾讯接口  5分钟5m   60分钟60m
         ...
         try:    return get_price_sina(  xcode,end_date=end_date,count=count,frequency=frequency)   #主力
             ...
             try: return get_price_min_tx(xcode,end_date=end_date,count=count,frequency=frequency)   #备用
             except:
                 try:                                                           #mootdx备用
                     from lib.sources_mootdx import get_kline
                     freq_map = {'5m': 0, '15m': 1, '30m': 2, '60m': 3}
                     return get_kline(xcode, frequency=freq_map.get(frequency, 3), offset=count)
                 except:
                     raise RuntimeError(f"所有数据源均失败: {code}")
