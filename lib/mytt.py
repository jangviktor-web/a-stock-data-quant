...
# V3.4  2026-07-21 同步上游BARSSINCEN修正(NaN), 新增SAR/TDX_SAR/SUMBARSFAST/DSMA/变窗口HHV_LL, 补充中文名
...
def BARSSINCEN(S, N):                  # N周期内第一次S条件成立到现在的周期数,N为常量  by jqz1226, peakgao
...
def MACD(CLOSE,SHORT=12,LONG=26,M=9):             # MACD(指数平滑异同移动平均线) EMA的关系，S取120日，和雪球小数点2位相同
    DIF = EMA(CLOSE,SHORT)-EMA(CLOSE,LONG);  
    DEA = EMA(DIF,M);      MACD=(DIF-DEA)*2
    return RD(DIF),RD(DEA),RD(MACD)

def KDJ(CLOSE,HIGH,LOW, N=9,M1=3,M2=3):           # KDJ(随机指标)
    RSV = (CLOSE - LLV(LOW, N)) / (HHV(HIGH, N) - LLV(LOW, N)) * 100
    K = EMA(RSV, (M1*2-1));    D = EMA(K,(M2*2-1));        J=K*3-D*2
    return K, D, J

def RSI(CLOSE, N=24):                           # RSI(相对强弱指标) 和通达信小数点2位相同
    DIF = CLOSE-REF(CLOSE,1) 
    return RD(SMA(MAX(DIF,0), N) / SMA(ABS(DIF), N) * 100)  

def WR(CLOSE, HIGH, LOW, N=10, N1=6):            # WR(威廉指标/超买超卖)
    WR = (HHV(HIGH, N) - CLOSE) / (HHV(HIGH, N) - LLV(LOW, N)) * 100
    WR1 = (HHV(HIGH, N1) - CLOSE) / (HHV(HIGH, N1) - LLV(LOW, N1)) * 100
    return RD(WR), RD(WR1)

def BIAS(CLOSE,L1=6, L2=12, L3=24):              # BIAS(乖离率)
    BIAS1 = (CLOSE - MA(CLOSE, L1)) / MA(CLOSE, L1) * 100
    BIAS2 = (CLOSE - MA(CLOSE, L2)) / MA(CLOSE, L2) * 100
    BIAS3 = (CLOSE - MA(CLOSE, L3)) / MA(CLOSE, L3) * 100
    return RD(BIAS1), RD(BIAS2), RD(BIAS3)

def BOLL(CLOSE,N=20, P=2):                       # BOLL(布林带)
    MID = MA(CLOSE, N); 
    UPPER = MID + STD(CLOSE, N) * P
    LOWER = MID - STD(CLOSE, N) * P
    return RD(UPPER), RD(MID), RD(LOWER)    

def PSY(CLOSE,N=12, M=6):                        # PSY(心理线)
    PSY=COUNT(CLOSE>REF(CLOSE,1),N)/N*100
    PSYMA=MA(PSY,M)
    return RD(PSY),RD(PSYMA)

def CCI(CLOSE,HIGH,LOW,N=14):                    # CCI(顺势指标)
    TP=(HIGH+LOW+CLOSE)/3
    return (TP-MA(TP,N))/(0.015*AVEDEV(TP,N))
        
def ATR(CLOSE,HIGH,LOW, N=20):                    # ATR(真实波动幅度/平均真实波幅)
    TR = MAX(MAX((HIGH - LOW), ABS(REF(CLOSE, 1) - HIGH)), ABS(REF(CLOSE, 1) - LOW))
    return MA(TR, N)

def BBI(CLOSE,M1=3,M2=6,M3=12,M4=20):             # BBI(多空指标)
    return (MA(CLOSE,M1)+MA(CLOSE,M2)+MA(CLOSE,M3)+MA(CLOSE,M4))/4    

def DMI(CLOSE,HIGH,LOW,M1=14,M2=6):               # DMI(趋向指标/动向指标) 结果和同花顺，通达信完全一致
...
def TAQ(HIGH,LOW,N):                               # TAQ(唐安奇通道/海龟交易) 大道至简，能穿越牛熊
    UP=HHV(HIGH,N);    DOWN=LLV(LOW,N);    MID=(UP+DOWN)/2
    return UP,MID,DOWN

def KTN(CLOSE,HIGH,LOW,N=20,M=10):                 # KTN(肯特纳交易通道) N选20日，ATR选10日
    MID=EMA((HIGH+LOW+CLOSE)/3,N)
    ATRN=ATR(CLOSE,HIGH,LOW,M)
    UPPER=MID+2*ATRN;   LOWER=MID-2*ATRN
    return UPPER,MID,LOWER       
  
def TRIX(CLOSE,M1=12, M2=20):                      # TRIX(三重指数平滑平均线)
    TR = EMA(EMA(EMA(CLOSE, M1), M1), M1)
    TRIX = (TR - REF(TR, 1)) / REF(TR, 1) * 100
    TRMA = MA(TRIX, M2)
    return TRIX, TRMA

def VR(CLOSE,VOL,M1=26):                            # VR(成交量比率/容量比率)
    LC = REF(CLOSE, 1)
    return SUM(IF(CLOSE > LC, VOL, 0), M1) / SUM(IF(CLOSE <= LC, VOL, 0), M1) * 100
  
def CR(CLOSE,HIGH,LOW,N=20):                        # CR(价格动量指标)
    MID=REF(HIGH+LOW+CLOSE,1)/3;
    return SUM(MAX(0,HIGH-MID),N)/SUM(MAX(0,MID-LOW),N)*100  

def EMV(HIGH,LOW,VOL,N=14,M=9):                     # EMV(简易波动指标)
    VOLUME=MA(VOL,N)/VOL;       MID=100*(HIGH+LOW-REF(HIGH+LOW,1))/(HIGH+LOW)
    EMV=MA(MID*VOLUME*(HIGH-LOW)/MA(HIGH-LOW,N),N);    MAEMV=MA(EMV,M)
    return EMV,MAEMV


def DPO(CLOSE,M1=20, M2=10, M3=6):                  # DPO(区间震荡线)
    DPO = CLOSE - REF(MA(CLOSE, M1), M2);    MADPO = MA(DPO, M3)
    return DPO, MADPO

def BRAR(OPEN,CLOSE,HIGH,LOW,M1=26):                 # BRAR(情绪指标/ARBR)
    AR = SUM(HIGH - OPEN, M1) / SUM(OPEN - LOW, M1) * 100
    BR = SUM(MAX(0, HIGH - REF(CLOSE, 1)), M1) / SUM(MAX(0, REF(CLOSE, 1) - LOW), M1) * 100
    return AR, BR

def DFMA(CLOSE,N1=10,N2=50,M=10):                    # DFMA(平行线差指标) 通达信叫DMA，同花顺叫新DMA
    DIF=MA(CLOSE,N1)-MA(CLOSE,N2); DIFMA=MA(DIF,M)
    return DIF,DIFMA

def MTM(CLOSE,N=12,M=6):                             # MTM(动量指标)
    MTM=CLOSE-REF(CLOSE,N);         MTMMA=MA(MTM,M)
    return MTM,MTMMA

def MASS(HIGH,LOW,N1=9,N2=25,M=6):                   # MASS(梅斯线)
    MASS=SUM(MA(HIGH-LOW,N1)/MA(MA(HIGH-LOW,N1),N1),N2)
    MA_MASS=MA(MASS,M)
    return MASS,MA_MASS
  
def ROC(CLOSE,N=12,M=6):                             # ROC(变动率指标)
    ROC=100*(CLOSE-REF(CLOSE,N))/REF(CLOSE,N);    MAROC=MA(ROC,M)
    return ROC,MAROC  

def EXPMA(CLOSE,N1=12,N2=50):                        # EXPMA(指数平均数)
    return EMA(CLOSE,N1),EMA(CLOSE,N2);

def OBV(CLOSE,VOL):                                  # OBV(能量潮指标)
    return SUM(IF(CLOSE>REF(CLOSE,1),VOL,IF(CLOSE<REF(CLOSE,1),-VOL,0)),0)/10000

def MFI(CLOSE,HIGH,LOW,VOL,N=14):                    # MFI(资金流量指标/成交量RSI)
    TYP = (HIGH + LOW + CLOSE)/3
    V1=SUM(IF(TYP>REF(TYP,1),TYP*VOL,0),N)/SUM(IF(TYP<REF(TYP,1),TYP*VOL,0),N)  
    return 100-(100/(1+V1))     
  
def ASI(OPEN,CLOSE,HIGH,LOW,M1=26,M2=10):            # ASI(振动升降指标)
...
def XSII(CLOSE, HIGH, LOW, N=102, M=7):              # XSII(薛斯通道II)
...
#------------------   3级：高级函数 (V3.4 新增, 来自 MyTT_plus.py) ------------------------------

def HHV_VAR(S, N):                                    #变窗口HHV(最高值) N支持序列
    """HHV变窗口版本，N可以是数组（每个bar不同窗口）"""
    if isinstance(N, (int, float)):
        return pd.Series(S).rolling(int(N)).max().values
    N = np.array(N)
...
def LLV_VAR(S, N):                                    #变窗口LLV(最低值) N支持序列
    """LLV变窗口版本，N可以是数组（每个bar不同窗口）"""
    if isinstance(N, (int, float)):
        return pd.Series(S).rolling(int(N)).min().values
    N = np.array(N)
    res = np.repeat(np.nan, len(S))
    for i in range(len(S)):
        if (not np.isnan(N[i])) and N[i] <= i + 1:
...
def SUMBARSFAST(X, A):                                #累加到阈值周期数(换手天数)
    """
    通达信SUMBARS函数: 将X向前累加直到>=A, 返回区间周期数
    例: SUMBARSFAST(VOL, CAPITAL) 求完全换手的周期数
    """
    if any(np.array(X) <= 0):
        raise ValueError('数组X的每个元素都必须大于0')
    X = np.flipud(np.array(X, dtype=float))
    length = len(X)
    if isinstance(A, (int, float)):
        A = np.repeat(float(A), length)
    A = np.flipud(np.array(A, dtype=float))
...
def DSMA(X, N):                                       #偏差自适应移动平均线
    """
    Deviation Scaled Moving Average (DSMA)
    自动根据市场波动调节平滑因子，比固定EMA更自适应
    """
...
def SAR(HIGH, LOW, N=10, S=2, M=20):                  #抛物转向指标(停损转向)
    """
    抛物转向 SAR(N,S,M): N日周期, S步长(%), M极限(%)
    例: SAR(10,2,20) = 10日抛物转向，步长2%，极限20%
    """
    HIGH = np.array(HIGH, dtype=float)
    LOW = np.array(LOW, dtype=float)
    f_step = S / 100; f_max = M / 100
    af = 0.0
...
def TDX_SAR(High, Low, iAFStep=2, iAFLimit=20):       #通达信精确SAR(停损转向)
    """
    通达信SAR算法，结果与通达信SAR完全一致
    :param High: 最高价序列
    :param Low: 最低价序列
    :param iAFStep: AF步长(%)
    :param iAFLimit: AF极限(%)
