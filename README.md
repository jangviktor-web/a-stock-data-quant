...
**A股量化分析工具箱（A-Share Data Quant）** 是一个开源的 **Python 股票数据分析与量化研究引擎**，面向 **A股（沪深京）** 与 **港股** 投资者、量化交易开发者、金融数据分析师。基于 **akshare + MyTT** 构建，一套 CLI 覆盖从数据获取到研究决策的完整链路：

- **行情数据**：A股 / 港股 / ETF / 指数 / 可转债 / 期货 / 期权实时行情与 K 线，多数据源（腾讯、东方财富、新浪、通达信、百度）自动降级，保证数据可用性。
- **技术分析**：20+ 技术指标（MACD、RSI、KDJ、BOLL、CCI、ATR、OBV 等）与形态识别（W 底、V 型反转、杯柄、三重底、回踩买入）。
- **策略回测**：7 种内置策略（均线、MACD、RSI、BOLL、KDJ、多策略共振），输出收益率、最大回撤、夏普比率、胜率与 HTML 图表。
- **个股研究**：筹码分布、板块资金流、F10 财务、估值分位、市场温度、北向资金、龙虎榜、融资融券、限售解禁、研报公告。
- **多市场扩展**：CN 期货主连、CN 期权（ETF + CFFEX 指数期权）、宏观数据（CPI / PPI / GDP / LPR / 社融）与 A 股公告事件。
- **AI 集成**：可作为 AI Agent 技能（SkillHub / ClawHub）加载，支持自然语言选股与研报写作工作流（读年报、深度报告、业绩快评、研报摘要等）。

**适用场景**：股票技术分析、量化策略研究、投资组合研究、金融数据爬取替代、AI 金融应用开发。

**GitHub 搜索关键词**：A股 量化分析、股票数据分析、技术指标、MACD、RSI、策略回测、量化回测、形态识别、实时行情、多股对比、筹码分布、资金流、F10 财务、估值分位、a-share、stock analysis、quant、backtest、technical analysis、trading strategy、akshare、pandas、python 金融。
