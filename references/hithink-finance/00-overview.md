...
| 场景 | 首选 | 一级入口 |
| --- | --- | --- |
| 人类终端、Agent 执行、自动化、远端与本地数据一体化 | CLI | [cli.md](cli.md) |
| Chat/IDE 会话已连接托管服务 | MCP | [mcp.md](mcp.md) |
| 零依赖 HTTP、自定义脚本、服务端集成 | REST API | [api.md](api.md) |
| Python、Notebook、研究流程或已有 marketdb | Python SDK | [python-sdk.md](python-sdk.md) |
...
全部缺失时，根据当前平台给出 [CLI 安装与配置入口](cli/setup.md) 中的全局环境变量指引，并使用以下说明：
...
- CLI 刚安装、统一凭据刚配置或更新、或 CLI 认证失效但统一凭据有效时，按 [CLI setup](cli/setup.md) 通过 `--api-key-stdin` 安全登录；已有 CLI 凭据需要同步时使用 `--replace`，不先 logout。
...
- CLI 不存在、版本异常、认证未配置或内置 Skills 不完整：进入 [CLI 入口](cli.md)。
- MCP 未连接、认证失败或需要识别工具意图：进入 [MCP 入口](mcp.md)。
- REST 参数、字段或错误码不明确：进入 [API 入口](api.md)。
- Python 安装、远端 toolkit 或本地 marketdb 问题：进入 [Python SDK 入口](python-sdk.md)。
